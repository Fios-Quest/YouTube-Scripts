...
Traits describe common behaviour between types that implement the trait.
...
If we limit what types `S` can be to only those that implement `ToString`, then we can now use the methods (or in this case method) that that trait provides because we know only types that implement that method can end up in this function. 

It takes one parameter that we've made generic, but we're limiting what types can be

For example, we could pass in string slices, numbers, even other Strings as they all implement `ToString`, however, `Vec` does not, so this won't compile.

---

We can also implement `ToString` on our own types. Imagine we have a **ehm** poorly designed Person type with a first and last name.

You can see that this Person can not be passed into our say Hello function as it is because it doesn't implement `ToString`.
...
We can now use Person inside of say_hello.
...
Often when people create libraries they'll make their own prelude module that contains the most commonly used types and traits so that you can import the entire prelude module rather than having to import a
...
The first problem we were having was that we can't access anything from Cat inside our emotional states.
...
In `main.rs` we'll add `mod animal` and then create the file `animal/mod.rs`.

Let's move `cat.rs` to `animal/cat.rs` so that it's a submodule of `animal`.

Finally, we need to rememebt add `pub mod cat` to `animal/mod.rs` and to update our use statement in `main.rs` to `animal::cat::Cat`.

We're now ready to make our trait, which we'll put in `animal/mod.rs`

With trait methods, we don't _have_ to define any behaviour (though we can), we only need to tell Rust how the method will be used.
...
It'd be better to call the underlying `Cat::get_name` from `Animal::get_name`, but how do we do that?
...
This is because when we call a method with the dot syntax, we call it on a specific instance, so Rust, like many similar languages, can infer the value of `self` (or `this` in some languages) to be the instance the method was called on.
...
There's a bit of a trick here though, when we add the trait bound to the implementation block, it actually goes on the implementation's generic listing, as the other generics are calls back to this one.
...
We can start by adding `pub mod human;` to `animal.mod`. Now we can add our Human type in here, we'll keep it pretty simple for now.
...
If you get stuck, I've implemented the code in this videos chapter of the IRISS book, just scroll to the bottom and hit the eye icon.
...
Finally, there's now a Discord server, so check the description and come join our growing community!
